...
- **Semantic discovery:** optional SigLIP image/text inference uses the same authenticated loopback-only worker. Source-derived pixels, semantic queries, binary embeddings, similarity scores and grouping proposals remain local. No face/identity recognition, sensitive-attribute labelling, cloud vector service, semantic telemetry or automatic metadata/organisation mutation is used. Model weights remain outside catalogues under `D:\AI Models\Keepframe`.
...
- **XMP sidecars and portable catalogue JSON:** created only after an explicit local action. They can contain filenames, managed-relative or original paths, hashes, tags, triage state, Develop recipes (including compressed accepted semantic-mask coverage and model provenance), derived-version provenance, GPS coordinates and explicit semantic suggestion Accept/Dismiss decisions. Embeddings, semantic queries, indexes and model files are excluded. Keepframe does not upload these exports, but anyone you share them with can read that metadata.
