...
describe("Triage adjustments", () => {
  it("exposes protected range controls in the main viewer and saves a candidate", async () => {
    const autoSettings = { ...neutralAdjustments, dynamicRange: 100, contrast: 8, highlights: -30, shadows: 20, whites: 6, blacks: -7, colourBoost: 9 };
    const auto = vi.fn().mockResolvedValue(autoSettings);
    ...
    expect(preview).toHaveBeenCalledWith(demoAssets[0], autoSettings);
    ...
    expect(photograph).toHaveAttribute("src", "adjusted-preview.png");
    await waitFor(() => expect(preview).toHaveBeenLastCalledWith(demoAssets[0], { ...autoSettings, exposure: 0.5 }));
    ...
    await waitFor(() => expect(apply).toHaveBeenCalledWith(demoAssets[0], { ...autoSettings, exposure: 0.5 }));
