...
export function MaskOverlay({ mask, settings, sourceAspect, tool, brush, showCoverage, onCommit, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    const move = (event: PointerEvent) => {
      ...
      if (drag.kind === "brush" && (next.geometry.kind === "brush" || next.geometry.kind === "semantic")) {
        const strokes = next.geometry.kind === "brush" ? next.geometry.strokes : next.geometry.refinements;
        const stroke = strokes.at(-1); if (stroke) { const previous = stroke.points.at(-1); if (!previous || Math.hypot(point.x - previous.x, point.y - previous.y) > .002) stroke.points.push(point); }
      }
  ...
  const begin = (kind: Drag["kind"]) => (event: ReactPointerEvent) => { event.preventDefault(); event.stopPropagation(); const point = fromEvent(event.nativeEvent); const original = structuredClone(draft);
    ...
    if (kind === "brush" && original.geometry.kind === "semantic") original.geometry.refinements.push({ points: [point], radius: brush.radius, feather: brush.feather, flow: brush.flow, erase: tool === "subtract" });
  ...
  const origin = semantic ? shown({ x: 0, y: 0 }) : null; const basisX = semantic ? shown({ x: 1, y: 0 }) : null; const basisY = semantic ? shown({ x: 0, y: 1 }) : null;
  const semanticTransform = origin && basisX && basisY ? `matrix(${basisX.x - origin.x} ${basisX.y - origin.y} ${basisY.x - origin.x} ${basisY.y - origin.y} ${pct(origin.x)} ${pct(origin.y)})` : undefined;
  const strokes = brushing?.strokes ?? semantic?.refinements ?? [];
  return <svg ref={svg} className={`mask-overlay ${tool ? "drawing" : ""}`} viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="Active mask overlay" onPointerDown={tool === "linear" || tool === "radial" ? begin("create") : tool === "brush" || tool === "erase" || tool === "add" || tool === "subtract" ? begin("brush") : undefined}>
    <defs>{linear && start && end ? <linearGradient id={`gradient-${draft.id}`} gradientUnits="userSpaceOnUse" x1={pct(start.x)} y1={pct(start.y)} x2={pct(end.x)} y2={pct(end.y)}><stop offset="0" stopColor="#e95858" stopOpacity="0" /><stop offset="1" stopColor="#e95858" stopOpacity={showCoverage ? .48 : .18} /></linearGradient> : null}<filter id={`semantic-${draft.id}`} colorInterpolationFilters="sRGB"><feColorMatrix values="0 0 0 0 .914 0 0 0 0 .345 0 0 0 0 .345 .333 .333 .333 0 0" /></filter></defs>
    ...
    {semantic ? <image href={`data:image/png;base64,${semantic.coveragePng}`} width="100" height="100" preserveAspectRatio="none" transform={semanticTransform} opacity={showCoverage ? .55 : .2} filter={`url(#semantic-${draft.id})`} /> : null}
    {strokes.map((stroke, index) => <polyline key={index} points={stroke.points.map((point) => { const value = shown(point); return `${pct(value.x)},${pct(value.y)}`; }).join(" ")} fill="none" stroke={stroke.erase ? "#74a7ff" : "#e95858"} strokeOpacity={showCoverage ? Math.max(.2, stroke.flow * .6) : .25} strokeWidth={stroke.radius * 200 / Math.max(settings.cropWidth, settings.cropHeight)} strokeLinecap="round" strokeLinejoin="round" />)}
  </svg>;
}
