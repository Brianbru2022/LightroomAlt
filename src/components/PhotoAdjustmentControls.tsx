    ...
    <div className={`photo-adjustment-controls ${compact ? "compact" : ""}`}>
      ...
      <details className="adjustment-group">
        ...
        <div className="adjustment-group-grid">
          ...
          <AdjustmentSlider label="Crop width" value={value.cropWidth} min={0.05} max={1} step={0.01} onChange={(next) => onChange("cropWidth", Math.min(next, 1 - value.cropLeft))} />
          <AdjustmentSlider label="Crop height" value={value.cropHeight} min={0.05} max={1} step={0.01} onChange={(next) => onChange("cropHeight", Math.min(next, 1 - value.cropTop))} />
          {onReplace ? <button type="button" className="reset-geometry-button" onClick={() => onReplace({ ...value, cropLeft: 0, cropTop: 0, cropWidth: 1, cropHeight: 1, rotateQuadrants: 0, straighten: 0 })}>Reset crop & orientation</button> : null}
        </div>
      </details>
    </div>
  );
